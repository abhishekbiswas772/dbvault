...
class RedisDatabaseBackupManager(DatabaseBackupManager):

    class BackupError(Exception):
        pass

    _RDB_MAGIC = b"REDIS"
    _TEMP_BACKUP_PATH = "./backup_temp.rdb"

    def __init__(self) -> None:
        self.connection: redis_lib.Redis | None = None
    ...
    def connect(self, host: str, user: str, password: str, database_name: str) -> None:
        if not all([host, database_name]):
            raise ValueError("host and database_name are required")

        db_index = int(database_name) if database_name.isdigit() else 0
    ...
    def _run_redis_dump(self, output_path: str) -> str:
        command = ["redis-cli", "-h", self.host]
        if self.password:
            command += ["-a", self.password]
        command += ["--rdb", output_path]
    ...
    def backup(
        ...
        **kwargs: Any
        ...
        backup_path = (
            os.path.join(file_path, "backup.rdb")
            if file_path
            else self._TEMP_BACKUP_PATH
        )

        try:
            rdb_path = self._run_redis_dump(backup_path)

            if not self.validate(rdb_path):
                raise self.BackupError("Backup validation failed")

            compressed_path = self.compress(rdb_path)
    ...
    def validate(self, file_path: str) -> bool:
        ...
        if magic != self._RDB_MAGIC:
            raise self.BackupError(f"Invalid RDB file: bad magic bytes {magic!r}")
