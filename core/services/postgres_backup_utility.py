...
from core.helpers.blobstorage_uploader import upload_to_s3, upload_to_azure
...
class PostgresDatabaseBackupManager(DatabaseBackupManager):
    ...
    def _run_postgresdump(self, output_path: str) -> str:
        env = os.environ.copy()
    ...
    def backup(
        ...
        **kwargs: Any
        ...
        try:
            sql_path = self._run_postgresdump(backup_path)
    ...
    def validate(self, file_path: str) -> bool:
        ...
        try:
            subprocess.run(
                ["createdb", "-h", self.host, "-U", self.user, temp_db],
                env=env,
                check=True
            )
            ...
            with open(file_path, "rb") as f:
                subprocess.run(
                    ["psql", "-h", self.host, "-U", self.user, "-d", temp_db],
                    stdin=f,
                    env=env,
                    check=True
                )
            ...
            subprocess.run(
                ["dropdb", "-h", self.host, "-U", self.user, "--if-exists", temp_db],
                env=env
            )
